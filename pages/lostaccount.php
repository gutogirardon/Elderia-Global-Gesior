<img id="ContentBoxHeadline" class="Title" src="layouts/tibiacom/images/header/headline-lostaccount.gif" alt="Contentbox headline">
<?php
if(!defined('INITIALIZED'))
	exit;

if($config['site']['lost_acc'])
{
	if($action == '')
	{
						...
						if(check_password($new_pass))
						{
							if(check_mail($new_email))
							{
								...
								if($account->getCustomField('next_email') < time())
								{
									...
									{
										$main_content .= '<br /><small>Account changed successfullyS.';
									}
								}
								else
								{
